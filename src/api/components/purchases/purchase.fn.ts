...
export const checkDataReqReceipt = (
  ...
  const totalConcepts = roundNumber(
    receiptConcepts.reduce((acc, concept) => acc + Number(concept.amount), 0),
    2,
  );
  ...
  const purchaseEntriesConcepts: IPurchaseEntries[] = receiptConcepts.map(
    (concept) => {
      return {
        date: headerReceipt.date,
        receipt_id: 0,
  ...
  const purchaseEntriesPayment: IPurchaseEntries[] = paymentReceipt.map(
    (payment) => {
      return {
        date: headerReceipt.date,
        receipt_id: 0,
  ...
  const purchaseEntries: IPurchaseEntries[] = [
...
export const createPurchaseTxtItem = (purchaseItems: Receipt) => {
  ...
  const providerDocType = stringFill(
    purchaseItems.dataValues.Provider?.document_type.toString() || '',
    2,
  );
  const providerNumber = stringFill(
    purchaseItems.dataValues.Provider?.document_number.toString() || '',
    20,
  );
  const providerName = stringFill(
    purchaseItems.dataValues.Provider?.business_name || ''.toString(),
    30,
    ' ',
    false,
  );
  ...
  const totalVat =
    purchaseItems.dataValues.VatRateReceipts?.reduce(
      (acc, vat) => acc + vat.vat_amount,
      0,
    ) || 0;
...
export const generateUncheckedReceiptsCVS = (
  ...
  const data: {
    ...
    const creditsNotes = [3, 8, 13, 53, 110, 119];
    const negativePositive = creditsNotes.includes(receipt.invoice_type_id)
      ? -1
      : 1;

    const invoiceType = Number(receipt.invoice_type_id);
    const totalVat: number = Number(
      (receipt.VatRatesReceipts
        ? receipt.VatRatesReceipts.reduce(
            (acc, vat) =>
              acc +
              Number(
                vat.vat_amount
    ...
    return {
      ...
      neto_iva_0:
        negativePositive *
        (receipt.VatRatesReceipts
          ? receipt.VatRatesReceipts.find((vat: any) => vat.vat_type_id === 3)
              ?.recorded_net || 0
          : 0),
      neto_iva_2_5:
        negativePositive *
      ...
      iva_27:
        ...
        (receipt.VatRatesReceipts
          ? receipt.VatRatesReceipts.find((vat: any) => vat.vat_type_id === 6)
              ?.vat_amount || 0
          : 0),
      neto_gravado: negativePositive * totalNet,
      iva_total: negativePositive * totalVat,
    };
  });
...
export const receiptsExcelGenerator = (receipts: IReceipts[]) => {
  const receiptsTraslated = receipts.map((receipt) => {
    ...
    return {
      ...
      'Operaciones Exentas':
        roundNumber(receipt.exempt_transactions) * negativePositive,
      'Percepciones de IVA':
        roundNumber(receipt.vat_withholdings) * negativePositive,
      'Percepciones de Impuestos Nacionales':
        roundNumber(receipt.national_tax_withholdings) * negativePositive,
      'Percepciones de Ingresos Brutos':
        roundNumber(receipt.gross_income_withholdings) * negativePositive,
      'Percepciones Municipales':
        roundNumber(receipt.local_tax_withholdings) * negativePositive,
      'Impuestos Internos':
        roundNumber(receipt.internal_tax) * negativePositive,
      'Total Iva 0%':
        roundNumber(
          receipt.VatRateReceipts
      ...
      'Total No Gravado': roundNumber(receipt.unrecorded) * negativePositive,
      Total: roundNumber(receipt.total) * negativePositive,
...
export const resumeDataGenerator = async (receipts: IReceipts[]) => {
  ...
  receipts = receipts.map((receipt) => {
    ...
    if (negativePositive === -1) {
      console.log('receipt :>> ', receipt.total);
    }
    receipt.total = receipt.total * negativePositive;
    receipt.exempt_transactions =
      receipt.exempt_transactions * negativePositive;
    receipt.vat_withholdings = receipt.vat_withholdings * negativePositive;
    receipt.national_tax_withholdings =
      receipt.national_tax_withholdings * negativePositive;
    receipt.gross_income_withholdings =
      receipt.gross_income_withholdings * negativePositive;
    receipt.local_tax_withholdings =
      receipt.local_tax_withholdings * negativePositive;
    receipt.internal_tax = receipt.internal_tax * negativePositive;
    receipt.unrecorded = receipt.unrecorded * negativePositive;
    receipt.VatRateReceipts =
      receipt.VatRateReceipts &&
      receipt.VatRateReceipts.map((vat) => {
        vat.recorded_net = vat.recorded_net * negativePositive;
        vat.vat_amount = vat.vat_amount * negativePositive;
        return vat;
      });
  ...
  const total = roundNumber(
    receipts.reduce((acc, receipt) => acc + Number(receipt.total), 0),
  );
  const exempt_transactions = roundNumber(
    receipts.reduce(
      (acc, receipt) => acc + Number(receipt.exempt_transactions),
      0,
    ),
  );
  const vat_withholdings = roundNumber(
    receipts.reduce(
      (acc, receipt) => acc + Number(receipt.vat_withholdings),
      0,
    ),
  );
  const national_tax_withholdings = roundNumber(
    receipts.reduce(
      (acc, receipt) => acc + Number(receipt.national_tax_withholdings),
      0,
    ),
  );
  const gross_income_withholdings = roundNumber(
    receipts.reduce(
      (acc, receipt) => acc + Number(receipt.gross_income_withholdings),
      0,
    ),
  );
  const local_tax_withholdings = roundNumber(
    receipts.reduce(
      (acc, receipt) => acc + Number(receipt.local_tax_withholdings),
      0,
    ),
  );
  const internal_tax = roundNumber(
    receipts.reduce((acc, receipt) => acc + Number(receipt.internal_tax), 0),
  );
  ...
  const recorded = roundNumber(
    receipts.reduce((acc, receipt) => {
      return (
        acc +
        Number(
          receipt.VatRateReceipts?.map((vat) => vat.recorded_net).reduce(
            (acc, net) => acc + Number(net),
            0,
          ) || 0,
        )
      );
...
export const completeReceipt = async (
  ...
  return await Promise.all(
    receipts.map(async (receipt) => {
      ...
      const receiptConcepts: IReceiptConcept[] =
        await ProviderParameter.findAll({
          where: [
      ...
      const paymentsParameters: IPaymentReceiptReq[] = await paymentParameter(
        periodId,
      ).then((data) => {
        return data
          .map((payment, key) => {
