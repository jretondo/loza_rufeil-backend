...
export const insertClientsParams = async (req: Request, res: Response, next: NextFunction) => {
    (async function (clientId: number, params: any, periodId: number) {
        const vatParams: [ISellsParameters] = params.vat.map((vatParam: ISellsParameters) => {
            return {
                client_id: clientId,
                type: vatParam.type,
        ...
        const othersParams: [ISellsParameters] = params.others.map((otherParam: ISellsParameters) => {
            return {
                client_id: clientId,
        ...
        await SellsParameter.destroy({ where: [{ client_id: clientId }, { accounting_period_id: periodId }] })
...
export const insertPaymentsParametersClient = async (req: Request, res: Response, next: NextFunction) => {
    (async function (clientId: number, params: any, periodId: number) {
        const paymentsParams: [IPointsSells] = params.map((paymentParam: IPointsSells) => {
            return {
                client_id: clientId,
        ...
        await PointsSells.destroy({ where: [{ client_id: clientId }, { accounting_period_id: periodId }] })
...
export const insertPeriod = async (req: Request, res: Response, next: NextFunction) => {
    (async function (month: number, year: number, periodId: number) {
        return await SellPeriod.create({ month, year, accounting_period_id: periodId })
...
export const upsertReceipt = async (req: Request, res: Response, next: NextFunction) => {
    (async function (
        ...
        const accountingPeriod = await SellPeriod.findOne({ where: { id: purchasePeriodId } })
        const providerAccount = await CustomerParameter.findAll({
            where: [{ customer_id: provider.id }, { accounting_period_id: accountingPeriod?.dataValues.accounting_period_id }]
        })
        ...
        if (newReceipt.dataValues.id) {
            const newVatRates = newRecords.VatRatesReceipts.length > 0 ? await VatRateInvoice.bulkCreate(newRecords.VatRatesReceipts.map(vatRate => {
                return {
                    ...vatRate,
            ...
            const newPurchaseEntries = await SellEntry.bulkCreate(newRecords.purchaseEntries.map(purchaseEntry => {
                return {
                    ...purchaseEntry,
                    receipt_id: newReceipt.dataValues.id || 0
                }
...
export const createPurchaseTxt = async (req: Request, res: Response, next: NextFunction) => {
    (async function (purchasePeriodId: number) {
        ...
        const purchasePeriod = await SellPeriod.findOne({ where: { id: purchasePeriodId } })
    ...
    (async function (purchasePeriodId: number) {
        ...
        const total = receipts.reduce((total, receipt) => {
            return {
                ...
                Total_Grabado: total.Total_Grabado + Number(receipt.dataValues.VatRatesInvoices?.map((vatRate:any) => vatRate?.recorded_net).reduce((total:any, recorded:any) => total + (recorded || 0), 0) || 0),
                ...
                Tota_IVA: total.Tota_IVA + Number((receipt.dataValues.VatRatesInvoices?.map((vatRate:any) => vatRate?.vat_amount).reduce((total:any, vatAmount:any) => total + (vatAmount || 0), 0) || 0))
            }
        }, {
            Total: 0,
            Total_No_Grabado: 0,
...
export const closePeriod = async (req: Request, res: Response, next: NextFunction) => {
    (async function (
        ...
        return await SellPeriod.update({ closed: true }, {
            where: [
                { id: purchasePeriodId },
                { accounting_period_id: accountingPeriodId }
            ]
